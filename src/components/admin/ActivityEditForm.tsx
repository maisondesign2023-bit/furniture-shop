...
export default function ActivityEditForm({ activity }: { activity: Activity }) {
  ...
  const [existingCoverUrl, setExistingCoverUrl] = useState(activity.cover_image);
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const { error: updateError } = await supabase
        ...
        .update({
          title,
          slug: slugify((form.get("slug") as string) || title, "activity"),
          ...
          published_at:
            status === "published" ? activity.published_at ?? new Date().toISOString() : activity.published_at,
        })
        .eq("id", activity.id);
  ...
  async function handleDelete() {
    if (!confirm(`確定要刪除「${activity.title}」這個活動嗎？此動作無法復原。`)) return;
    setDeleting(true);
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-2xl space-y-6 font-body text-sm">
      ...
      <Field label="網址代稱 slug（英文/數字，會自動轉換格式）">
        <input name="slug" defaultValue={activity.slug} className="input" />
      </Field>
        ...
        {existingCoverUrl && (
          <div className="mb-3 flex items-center gap-4">
            <div className="relative h-24 w-32 overflow-hidden border border-line bg-surface">
              <Image src={existingCoverUrl} alt={activity.title} fill sizes="128px" className="object-cover" />
            </div>
            <button
              type="button"
      ...
      <Field label="摘要（顯示在列表）">
        <textarea name="excerpt" defaultValue={activity.excerpt ?? ""} rows={2} className="input" />
      </Field>
      <Field label="內文">
        <RichTextEditor ref={editorRef} name="content" bucket="blog-images" initialValue={activity.content ?? ""} />
      </Field>
      <Field label="發布狀態（草稿不會出現在前台，整理好再切換成發布）">
        <select name="status" defaultValue={activity.status} className="input">
      ...
      <div className="border-t border-line pt-6">
        ...
        <Field label="SEO 標題">
          <input name="seo_title" defaultValue={activity.seo_title ?? ""} className="input" />
        </Field>
        <Field label="SEO 描述">
          <textarea name="seo_description" defaultValue={activity.seo_description ?? ""} rows={2} className="input" />
        </Field>
      </div>
