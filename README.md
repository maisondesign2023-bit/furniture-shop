...
<!-- deploy trigger: verifying updated NEXT_PUBLIC_SITE_NAME (Bova Living) -->
...
## 更新九：視覺風格調整 + 品牌名稱 + 商品上架人員追蹤

**視覺調整**
- 主背景改為白色（原本是米杏色）
- 全站拿掉斜體樣式，改用一般正體
- 主要標題（頁面大標）改用粗體，取代原本的斜體強調
- 品牌名稱改為「VERO Living」（原本是「木・室」）

**商品上架人員追蹤**

新增商品時，系統會自動記錄「是誰上架的」（登入的帳號），`/admin/products` 商品列表新增「上架人員」欄位可以看到。

**⚠️ 重要的架構限制，請先了解：**

目前後台的「管理員權限」是全有或全無：只要能登入 `/admin` 新增商品，就代表這個帳號同時也能看到所有訂單、所有會員資料、可以刪除商品、修改任何設定。也就是說，**如果你讓業務員的帳號可以上架商品，他們目前也會同時擁有查看全部訂單、會員資料等完整後台權限**，沒有辦法只給「上架商品」這一項權限。

如果你需要業務員只能上架/管理商品、但不能看訂單或會員資料，這需要另外設計一套「員工權限分級」機制（區分管理員 / 業務員兩種身分，並調整資料庫的安全規則），是一個新功能，工程量不小。先讓我知道你需不需要這個，我可以評估後幫你加上。

**設定步驟：**

1. Supabase SQL Editor 執行 `supabase/migration_08_product_uploader.sql`
2. 到 `/admin/members`，把每個業務員帳號的姓名欄位填上真實姓名（點擊姓名欄位直接輸入，會自動存檔），這樣「上架人員」那欄才會顯示有意義的名字
3. `NEXT_PUBLIC_SITE_NAME` 這個環境變數（本機 `.env.local` 跟 Cloudflare 都要改）建議也更新成 `VERO Living`，這樣網頁標題跟通知信的品牌名稱才會一致

## 更新十：圖文編輯器（部落格/案例/商品說明）

部落格文章、案例的「完整說明」、商品的「商品敘述」都改用圖文編輯器，可以：
- 在文字中插入圖片（會自動上傳到對應的 Storage bucket）
- 調整圖片大小（小/中/大/滿版）
- 調整圖片位置（置左/置中/置右，置左置右會讓文字環繞在圖片旁邊）
- 文字本身也可以置左/置中/置右
- 基本的粗體、斜體、標題、清單

**設定步驟：**

這次新增了套件依賴（Tiptap 編輯器），本機測試前記得先跑一次：
```
npm install
```
Cloudflare 那邊會在部署時自動安裝，不用額外設定。

**注意事項：**
- 這個編輯器目前開放給後台的管理員使用，內容是「信任內容」（不會被一般訪客的輸入影響），所以沒有做額外的內容過濾，純粹給你自己/業務員上架時使用
- 舊的商品敘述、部落格文章、案例說明（用純文字寫的）改版後仍然能正常顯示，只是無法用新的圖片編輯功能編輯裡面既有的文字內容——如果要用新功能重新編排，直接在編輯器裡重新貼上/編輯即可

## 更新十一：關於我們頁面改成後台可編輯

`/about`（關於我們）之前是寫死在程式碼裡的頁面，現在跟「運送與退換貨」「隱私權政策」一樣，改成從後台編輯，而且用的是圖文編輯器（可以放品牌故事照片、調整圖片大小位置）。

**設定步驟：**

1. Supabase SQL Editor 執行 `supabase/migration_09_about_page.sql`
2. Supabase → Storage，新增一個 **public** bucket：`site-images`（給「關於我們」等一般頁面的圖片用）
3. 到 `/admin/pages`，會看到多一筆「關於我們」，點進去編輯

另外，「運送與退換貨」「隱私權政策」這兩個頁面的編輯器，也一併升級成圖文編輯器了（原本是純文字框）。
